...
export class ApiUtil {
  ...
  static Timeslot(jstime) {

    // 计算出当前日期时间到之前的日期时间的毫秒数，以便进行下一步的计算
    //var time = jstime-(new Date()).getTime() / 1000 ;
 
    var startTime = new Date(); // 开始时间

    var endTime = new Date(jstime.replace(new RegExp(/-/gm), "/")); // 结束时间
    ...
    console.log(Math.floor(time / 1000),'秒'); 
    console.log(Math.floor(time / 1000 / 60), '分'); 
    console.log(Math.floor(time / 1000 / 60 / 60),'时');
    console.log(Math.floor(time / 1000 / 60 / 60 / 24),'天');

    //console.log(time,'扣款')

    var num = 0;
    if (time >= 31104000000) { // N年前
      num = parseInt(time / 1000 / 31104000);
      return num + '年';
    }
    if (time >= 2592000000) { // N月前
      num = parseInt(time / 1000 / 2592000);
      return num + '月';
    }
    if (time >= 86400000) { // N天前
      num = parseInt(time / 1000 / 86400);
      return num + '天';
    }
    if (time >= 3600000) { // N小时前
      num = parseInt(time / 1000 / 3600);
      return num + '小时';
    }
    if (time > 60000) { // N分钟前
      num = parseInt(time / 1000 / 60);
      return num + '分钟';
    }
     return '1分钟';
  }
